defmodule InvaderWeb.CoreComponents do
  @moduledoc """
  ...
  use Gettext, backend: InvaderWeb.Gettext
  ...
  @doc """
  Renders an arcade-styled modal dialog.

  ## Examples

      <.modal id="sprite-modal" show>
      ...
      >
        <div class="flex min-h-full items-center justify-center p-4">
          <div
            ...
            class="arcade-modal w-full max-w-lg"
          >
            <div class="flex items-center justify-between mb-4 border-b border-cyan-800 pb-3">
              <h2
                :if={@title != []}
                id={"#{@id}-title"}
                class="text-sm text-cyan-400 font-bold tracking-wider"
              >
                {render_slot(@title)}
              </h2>
              <button
            ...
            <div id={"#{@id}-content"} class="text-white text-xs">
              {render_slot(@inner_block)}
            </div>
          </div>
        </div>
  ...
  @doc """
  Renders an arcade-styled confirmation dialog.
  ...
  ## Examples
      ...
      <.confirm_dialog
        ...
        message="Are you sure you want to delete this sprite?"
        confirm_text="DELETE"
        on_confirm={JS.push("delete_sprite", value: %{id: @sprite.id})}
      />
  """
  attr :id, :string, required: true
  ...
  def confirm_dialog(assigns) do
    ...
    <.modal id={@id}>
      <:title>{@title}</:title>
      <div class="space-y-4">
        <p class="text-cyan-400 text-xs">{@message}</p>
        <div class="flex justify-end gap-3 mt-6">
          ...
          <button
            ...
            class={[
              "arcade-btn text-[10px]",
              @danger && "border-red-500 text-red-400",
              !@danger && "border-green-500 text-green-400"
            ]}
          >
            {@confirm_text}
          </button>
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(InvaderWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(InvaderWeb.Gettext, "errors", msg, opts)
    end
  end
